...
docker exec nodered sh -c "npm install --unsafe-perm /data/offline_modules/modules/node-xlsx-0.24.0.tgz"  >/dev/null
docker exec nodered sh -c "npm install --unsafe-perm /data/offline_modules/modules/formidable-3.5.4.tgz"  >/dev/null
